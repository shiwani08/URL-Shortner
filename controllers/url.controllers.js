...
// PATCH /:shortId/:id - Update userId for a URL
export async function handleUpdateUserId(req, res) {
  const { shortId, id: userId } = req.params; // shortId to identify URL, id is new userId

  try {
    const url = await Url.findOne({ shortId }); // find by shortId
    if (!url) {
      return res.status(404).json({ error: "URL not found" });
    }

    url.userId = userId; // update the userId
    await url.save();

    res.status(200).json({ message: "User ID updated successfully", url });
  } catch (error) {
    console.error("Failed to update userId for URL:", error);
    res.status(500).json({ error: "Failed to update userId" });
  }
}
